...
"""
MVP rule-based classifier + slot extractor.

Usage:
  from mvp_rules import predict_mvp

  out = predict_mvp(
      text="My dad is not breathing and unconscious",
...
def load_rules(path: str = "mvp_regex_dictionary.json") -> Dict[str, Any]:
    global _RULES
    if _RULES is None:
        _RULES = json.loads(Path(path).read_text(encoding="utf-8"))
    return _RULES
